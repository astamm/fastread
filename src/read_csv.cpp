...
List read_csv(Source source, List parser_spec, CharacterVector col_names,
              CharacterVector na_strings = NA_STRING,
              int n = 0, int skip = 0) {
...
// [[Rcpp::export]]
List read_csv_from_file(std::string path, List parser_spec, CharacterVector col_names,
                        CharacterVector na_strings = NA_STRING,
                        int n = 0, int skip = 0) {
    
    MMapSource<> source(path);
    return read_csv(source, parser_spec, col_names, na_strings, n, skip);
}

// [[Rcpp::export]]
List read_csv_from_connection(SEXP conn, List parser_spec, CharacterVector col_names,
                              CharacterVector na_strings = NA_STRING,
                              int n = 0, int skip = 0) {
